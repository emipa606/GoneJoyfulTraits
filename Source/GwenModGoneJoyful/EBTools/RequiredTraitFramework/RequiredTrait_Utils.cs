...
public class RequiredTrait_Utils
{
    ...
    {
        foreach (var td in traitDefs.OrEmptyIfNull())
        {
    ...
    /// <summary>
    ///     Returns true if <paramref name="pawn" /> has no <paramref name="forbidden" /> traits, all
    ///     <paramref name="reqAll" /> traits, and at least one <paramref name="reqOne" /> trait. Shorthand for overload with a
    ///     last boolean argument assuming the user put false.
    /// </summary>
    ...
    /// <returns></returns>
    public static bool IsAllowedByTraits(Pawn pawn, List<TraitDef> forbidden, List<TraitDef> reqAll,
        List<TraitDef> reqOne)
    {
    ...
    /// <summary>
    ///     Returns true if <paramref name="pawn" /> has no <paramref name="forbidden" /> traits, all
    ///     <paramref name="reqAll" /> traits, and at least one <paramref name="reqOne" /> trait. Ignore this last requirement
    ///     if <paramref name="ignoreRequiredOneTraits" /> is true.
    /// </summary>
    /// <param name="pawn"></param>
    /// <param name="forbidden"></param>
    /// <param name="reqAll"></param>
    ...
    /// <returns></returns>
    public static bool IsAllowedByTraits(Pawn pawn, List<TraitDef> forbidden, List<TraitDef> reqAll,
        List<TraitDef> reqOne, bool ignoreRequiredOneTraits)
    {
        return !HasRestrictedTrait(pawn, forbidden) && HasAllRequiredTraits(pawn, reqAll) &&
               (ignoreRequiredOneTraits || HasOneRequiredTrait(pawn, reqOne));
    }


    // Public functions
    /// <summary>
    ///     Given a <c>JoyGiver</c> and <c>Pawn</c>, returns the complete modifier for the float return value of the
    ///     GetChance() method from the <c>JoyGiver</c> instance. Includes both the forbid/allow and increase/decrease chance
    ///     modifiers.
    /// </summary>
    ...
    public static float GetChanceModifierFromTraits(JoyGiver joy, Pawn pawn)
    {
        ...
        return GetIncDecChanceModifierFromTraits(pawn, inc, dec) *
               IsAllowedByTraits(pawn, forbidden, reqAll, reqOne).AsInt();
    }

    /// <summary>
    ///     Given a <c>JoyGiver_TakeDrug</c> and <c>Pawn</c>, returns the complete modifier for the float return value of the
    ///     GetChance() method from the <c>JoyGiver_TakeDrug</c> instance. Includes both the forbid/allow and increase/decrease
    ///     chance modifiers.
    /// </summary>
    /// <param name="joy"></param>
    /// <param name="pawn"></param>
    ...
    /// <summary>
    ///     Given a <c>JoyGiver_Skygaze</c> and <c>Pawn</c>, returns the complete modifier for the float return value of the
    ///     GetChance() method from the <c>JoyGiver_Skygaze</c> instance. Includes both the forbid/allow and increase/decrease
    ///     chance modifiers.
    /// </summary>
    /// <param name="joy"></param>
    /// <param name="pawn"></param>
    ...
    /// <summary>
    ///     Gets the increase or decrease modifier for the GetChance() method based on the traits contained within
    ///     <paramref name="pawn" />'s <paramref name="inc" /> and <paramref name="dec" /> <c>TraitDef</c> lists. Doubles the
    ///     modifier per trait in <paramref name="inc" /> and halves per trait in <paramref name="dec" />.
    /// </summary>
    /// <param name="pawn"></param>
